...
@dataclass
class KITML(BaseProcessorConfig):
    """Configuration for KIT Motion-Language metadata generation."""
    processed_dir: str = "data/KITML"
    save_dir: str = "data/processed_data/KITML"


class KITMLProcessor(AbstractSceneProcessor[KITML]):
    """Processor for generating metadata for KIT Motion-Language dataset."""
    
    def __init__(self, config: KITML):
    ...
    def _load_scene_list(self) -> List[str]:
        ...
        scene_ids = []
        for scene_file in Path(self.config.processed_dir).glob("*.xml"):
            scene_ids.append(scene_file.stem.split("_")[0])
    ...
    def _process_single_scene(self, scene_id: str) -> Dict[str, Any] | None:
        ...
        mmm_path = Path(processed_scene_base) / f"{scene_id}_mmm.xml"
        ann_path = Path(processed_scene_base) / f"{scene_id}_annotations.xml"
...
def main():
    parser = argparse.ArgumentParser(description="Process KIT Motion-Language metadata based on the original data.")
    
    # Arguments for input data locations
    parser.add_argument('--processed_dir', type=str, default=KITML.processed_dir, 
    ...
    parser.add_argument('--save_dir', type=str, default=KITML.save_dir, # Use base default initially
                        help=f'Directory to save the output JSON metadata (defaults to processed_dir: {KITML.processed_dir}).')
    ...
    processor = KITMLProcessor(config)
